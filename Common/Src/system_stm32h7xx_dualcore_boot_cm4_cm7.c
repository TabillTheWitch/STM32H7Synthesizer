...
{
    uint8_t* src = (uint8_t*)&_sitcm_load;
    uint8_t* dst = (uint8_t*)&_sitcm;

    while (dst < (uint8_t*)&_eitcm) {
        *dst++ = *src++;
    }
...
static void ITCM_Enable(void)
{
    // ITCM size = 64KB (STM32H755 固定)
    // SZ フィールドは「ITCM サイズを 64KB に設定する」＝ 0 を書く
    // BASEADDR は常に 0x00000000 にマップされる

    SCB->ITCMCR =
        (0U << SCB_ITCMCR_SZ_Pos) |   // ITCM size = 64KB
        SCB_ITCMCR_EN_Msk        |   // Enable ITCM
        SCB_ITCMCR_RMW_Msk       |   // Read-Modify-Write
        SCB_ITCMCR_RETEN_Msk;        // Retention in sleep
